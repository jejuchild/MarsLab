    ...
    >
      ...
      <div className="p-2">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          {scene.equirect_size_mb && (
            <span className="text-[10px] text-[#4a5a7a]">
              {scene.equirect_size_mb}MB
            </span>
          )}
        </div>
      </div>
...
export default function MastcamPanoPage() {
  ...
  const panoScenes = filtered.filter((s) => s.has_equirectangular);
  const previewOnly = filtered.filter((s) => !s.has_equirectangular);

  const handleSelect = useCallback((scene: Scene) => {
    setSelected(scene);
    setViewerMode(scene.has_equirectangular ? "panorama" : "preview");
  }, []);

  const imageUrl = selected
    ? viewerMode === "panorama" && selected.has_equirectangular
      ? `/api/mastcam/panorama/${selected.id}`
    ...
    <div className="h-screen flex flex-col bg-[#0a0f18] text-[#e2e8f0]">
      ...
      <header className="flex items-center justify-between px-4 py-2 border-b border-[#1e293b] bg-[#0d1420]/80 backdrop-blur shrink-0">
        <div className="flex items-center gap-3">
          <a href="/" className="text-[#6b7c9c] hover:text-[#c8d6e5] transition-colors text-sm">
            &larr; MarsLab
          </a>
          ...
          <span className="text-xs text-[#4a5a7a]">
            Perseverance Rover &middot; Jezero Crater
          </span>
        </div>
        <span className="text-xs text-[#4a5a7a]">
          {scenes.length} scenes &middot; {panoScenes.length + previewOnly.length} filtered
        </span>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-72 shrink-0 border-r border-[#1e293b] flex flex-col bg-[#0d1420]">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {loading ? (
              <p className="text-xs text-[#4a5a7a] text-center py-8">Loading scenes...</p>
            ) : filtered.length === 0 ? (
              <p className="text-xs text-[#4a5a7a] text-center py-8">No matches</p>
            ) : (
              filtered.map((s) => (
                <SceneCard
                  key={s.id}
          ...
          {!selected ? (
            <div className="flex-1 flex items-center justify-center text-[#4a5a7a]">
              <div className="text-center">
                <p className="text-lg">Select a panorama from the sidebar</p>
                <p className="text-xs mt-1">
                  Click a scene to view in 360&deg; or preview mode
                </p>
              </div>
            </div>
          ) : (
