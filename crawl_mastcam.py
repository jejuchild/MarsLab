...
def cube_to_equirectangular(face_images, output_width=None):
    """Convert 6 cube face images to an equirectangular panorama.
    face_images: dict with keys f,r,b,l,u,d -> PIL Images
    """
    ...
    faces = {}
    for key in FACES:
        faces[key] = np.array(face_images[key], dtype=np.float32)

    output = np.zeros((output_height, output_width, 3), dtype=np.uint8)

    # For each pixel in equirectangular, find corresponding cube face pixel
    for y in range(output_height):
        # latitude: pi/2 (top) to -pi/2 (bottom)
        lat = (0.5 - y / output_height) * math.pi
        for x in range(output_width):
            # longitude: -pi (left) to pi (right)
            ...
            # UV to pixel coords (u,v in [-1,1] -> [0, face_size-1])
            px = int((u + 1) / 2 * (face_size - 1))
            py = int((v + 1) / 2 * (face_size - 1))
            px = max(0, min(face_size - 1, px))
            py = max(0, min(face_size - 1, py))

            output[y, x] = faces[face_key][py, px].astype(np.uint8)
