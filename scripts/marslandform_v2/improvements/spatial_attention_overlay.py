#!/usr/bin/env python3
"""Generate spatial attention heatmap overlays on full HiRISE browse images.

For each test image, shows WHERE in the image the model detects the landform
by overlaying attention weights as a hot/cold heatmap on the original browse image.

Produces:
1. Per-image heatmap overlays (browse image + attention overlay)
2. Per-class gallery of localized landforms
3. Combined comparison showing original → attention → top-attention tiles
"""
import csv
import json
import sys
...
def load_tile_metadata():
    """Load tile_metadata.csv into dict: image_id -> list of {tile_idx, tile_row, tile_col, lat, lon, ...}."""
...
def get_predictions_with_attention(model, device):
    """Get predictions + full attention weights for all test images."""
...
def generate_attention_overlay(image_id, attention_weights, tile_meta_list, pred_cls, true_cls, confidence, correct, out_path):
    """Generate a single attention heatmap overlay on the browse image."""
    ...
    # Map attention weights to tile positions
    # Attention weights correspond to tiles in order of tile_idx
    n_att = len(attention_weights)
    n_meta = len(tile_meta_list)
    
    # Sort by tile_idx to match embedding order
    ...
    if valid_mask.any():
        att_min = np.nanmin(att_grid)
        att_max = np.nanmax(att_grid)
        if att_max > att_min:
    ...
    # Upsample attention grid to image resolution
    ...
    # Create figure with 3 panels: Original | Heatmap Overlay | Top-5 tiles
    fig = plt.figure(figsize=(20, 8))
    
    # Panel 1: Original browse image
    ax1 = fig.add_axes([0.01, 0.08, 0.30, 0.82])
    ax1.imshow(browse_arr)
    ax1.set_title("Original Browse Image", fontsize=11, fontweight="bold")
    ax1.axis("off")
    ...
    # Create custom colormap: transparent blue → yellow → red
    cmap = plt.cm.jet.copy()
    cmap.set_bad(alpha=0)
    
    # Mask zero-weight areas
    ...
    # Draw rectangles around top-5 attention tiles
    for rank, (att_w, tile_info) in enumerate(top5):
        y0 = tile_info["tile_row"] * TILE_SIZE
        x0 = tile_info["tile_col"] * TILE_SIZE
        rect = Rectangle((x0, y0), TILE_SIZE, TILE_SIZE, 
    ...
    ax2.set_title("Attention Heatmap Overlay\n(hot = high attention = landform evidence)", fontsize=11, fontweight="bold")
    ax2.axis("off")
    ...
    # Panel 3: Top-5 attention tiles
    ax3_base = fig.add_axes([0.68, 0.08, 0.30, 0.82])
    ax3_base.axis("off")
    ax3_base.set_title("Top-5 Attention Tiles\n(what the model focuses on)", fontsize=11, fontweight="bold")
    ...
    for rank, (att_w, tile_info) in enumerate(top5):
        ...
        if tile_path.exists():
            ...
            ax_tile.set_title(f"#{rank+1}: {att_pct:.1f}%", fontsize=8, fontweight="bold")
            ax_tile.axis("off")
            
            # Color border
            for spine in ax_tile.spines.values():
                spine.set_visible(True)
                spine.set_edgecolor("red" if rank == 0 else "orange")
    ...
    fig.suptitle(
        f"{image_id}  |  True: {true_cls}  |  Pred: {pred_cls} ({confidence:.0%})  |  {status}",
...
def generate_class_localization_gallery(results, tile_meta, out_dir):
    """Per-class gallery: 2x3 grid of attention heatmap overlays for each landform class."""
    print("Generating per-class localization galleries...")
    
    for cls in CLASS_ORDER[:4]:  # LDA, LVF, CCF, GLF (skip BG)
        ...
        fig.suptitle(f"Landform Localization: {cls} — Where the model detects {cls} features",
        ...
        for i in range(6):
            ...
            for j, tile_info in enumerate(sorted_meta):
                if j < len(r["attention"]):
                    att_grid[tile_info["tile_row"], tile_info["tile_col"]] = r["attention"][j]
            ...
            if valid.any():
                ...
                if att_max > att_min:
                    ...
                    att_norm = np.where(valid, 0.5, np.nan)
            else:
                ax.axis("off")
                continue
            ...
            for rr in range(att_grid.shape[0]):
                for cc in range(att_grid.shape[1]):
                    if np.isnan(att_norm[rr, cc]):
                        continue
                    y0, x0 = rr * TILE_SIZE, cc * TILE_SIZE
                    y1, x1 = min(y0 + TILE_SIZE, img_h), min(x0 + TILE_SIZE, img_w)
                    heatmap[y0:y1, x0:x1] = att_norm[rr, cc]
                    weight[y0:y1, x0:x1] = 1.0
            
            heatmap = gaussian_filter(heatmap, sigma=TILE_SIZE * 0.3)
            weight_s = gaussian_filter(weight, sigma=TILE_SIZE * 0.3)
            weight_s[weight_s == 0] = 1.0
            heatmap = heatmap / weight_s
            ...
            ax.imshow(browse_img)
            ax.imshow(heatmap_masked, cmap="jet", alpha=0.5, vmin=0, vmax=1)
            ax.set_title(f"{img_id[:20]}\nConf: {r['confidence']:.0%}", fontsize=9)
            ax.axis("off")
...
def main():
    print("=" * 60)
    print("SPATIAL ATTENTION HEATMAP OVERLAY")
    print("=" * 60)
    ...
    # Generate individual attention overlays for top-2 per class
    print("\nGenerating individual attention overlays...")
    generated = 0
    for cls in CLASS_ORDER:
        cls_results = [r for r in results if r["pred"] == cls and r["correct"]]
        cls_results.sort(key=lambda x: -x["confidence"])
        
        for rank, r in enumerate(cls_results[:3]):  # Top 3 per class
            img_id = r["image_id"]
            meta_list = tile_meta.get(img_id, [])
            
            out_path = loc_dir / f"heatmap_{cls}_{rank}_{img_id}.png"
            success = generate_attention_overlay(
                img_id, r["attention"], meta_list,
                r["pred"], r["true"], r["confidence"], r["correct"],
                out_path
            )
            if success:
                generated += 1
                print(f"  [{cls}] {img_id}: conf={r['confidence']:.2f} → {out_path.name}")
    
    # Also generate for misclassifications
    ...
    for rank, r in enumerate(wrong[:5]):
        img_id = r["image_id"]
        meta_list = tile_meta.get(img_id, [])
        out_path = loc_dir / f"heatmap_WRONG_{rank}_{r['true']}_as_{r['pred']}_{img_id}.png"
        success = generate_attention_overlay(
            img_id, r["attention"], meta_list,
            r["pred"], r["true"], r["confidence"], r["correct"],
            out_path
    ...
    print(f"\n  Total overlays generated: {generated}")
    
    # Generate per-class localization galleries
    generate_class_localization_gallery(results, tile_meta, out_dir)
    
    print(f"\n{'='*60}")
    print(f"All spatial attention overlays saved to:")
    print(f"  Individual: {loc_dir}/heatmap_*.png")
    print(f"  Galleries:  {out_dir}/localization_*.png")
    print(f"{'='*60}")
