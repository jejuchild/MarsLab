#!/usr/bin/env python3
"""Batch process ALL Mastcam-Z frames per sol (not just the first).

Each sol has ~24 unique observations (48 L/R pairs, we use L only).
This script downloads and SPICE-transforms all of them, then merges
into a single combined lon/lat map per sol.
"""
...
def process_sol_allframes(sol: int) -> dict:
    """Process all Mastcam-Z frames for a sol.

    Returns metadata dict with per-frame and combined results.
    """
    output_dir = OUTPUT_DIR / f"sol{sol:05d}"
    output_dir.mkdir(parents=True, exist_ok=True)
    combined_npz = output_dir / "combined_lonlat.npz"

    products = get_left_linearized_products(sol)
    if not products:
        raise ValueError(f"No left linearized XYZ products for sol {sol}")

    print(f"  Found {len(products)} unique frames")

    # Initialize SPICE once per sol
    init_spice(sol)

    frames = []
    all_lon = []
    all_lat = []
    all_px = []  # pixel x in the frame
    all_py = []  # pixel y in the frame
    all_frame_idx = []  # which frame this pixel belongs to
    ...
    for i, xyz_prod in enumerate(products):
        ...
        frame_npz = output_dir / f"frame_{i:03d}_lonlat.npz"
...
def main():
    ...
    results = {}
    results_path = OUTPUT_DIR / "allframes_results.json"
    ...
    for i, sol in enumerate(sols, 1):
        ...
        try:
            meta = process_sol_allframes(sol)
    ...
    print(f"\n{'='*60}")
    print(f"ALL-FRAMES BATCH COMPLETE!")
